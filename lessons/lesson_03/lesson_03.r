...
var$NIVEL_SOCIOECO <- factor(var$NIVEL_SOCIOECO, c('Bajo', 'Medio', 'Alto'), ordered = TRUE)
...
"Tabla de frecuencias - cualitativas, tabla de distribucion de frecuencias - cuantitativas"
...
# Moda: Valor con mayo frecuencia
Mode(df$total_day_calls)
# Only Mode
Mode(df$total_day_calls)[1]

# Medidas de dispersión
# Dispersiones respecto a la media
...
deciles <- quantile(df$total_day_calls, probs = c(0.1, 0.9, by = 0.1))
deciles
...
my_hist <- hist(var$INGRESO, breaks = bins, main = 'Histograma Intreso')
my_hist
...
"Otra método gráfica para visualizar la distribución de una variable es a través de 
